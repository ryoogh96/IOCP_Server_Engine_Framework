...
namespace Engine
{
	...
	bool DBSynchronizer::GatherDBIndexes()
	{
		...
		while (getDBIndexes.Fetch())
		{
			...
			// find column to mapping where index is on
	...
	void DBSynchronizer::CompareDBModel()
	{
		...
		// scan DB real Tables and compare with definition of xml file.
		...
		// add new XML Table definition where has not been deleted from map
	...
	void DBSynchronizer::CompareColumns(DBModel::TableRef dbTable, DBModel::ColumnRef dbColumn, DBModel::ColumnRef xmlColumn)
	{
		...
		// save related Index to delete later
